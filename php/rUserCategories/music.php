<html>
	<head>
		...
		<script type="text/javascript" src="../../js/catviewscript.js"></script>
	</head>
<body>
<?php 
...
$sql = "SELECT Story_ID FROM story WHERE Category_ID=1 ORDER BY Publish_Date DESC";
			...
			if ($result->	num_rows > 0) {
    			...
    			while($row = $result->fetch_assoc()) {
					...
					$like = $runStory['Like_Count'];
					$disLike = $runStory['Dislike_Feed'];
					echo "<main id='acc'>
							<section id='item.".$i."'>
								<a href='#item.".$i."' style='font-family:Verdana, Geneva, sans-serif'	>".$title."</a>
								<div id='postDet'>Posted by <b>".$author."</b><br>Date <b>".$publishDate."</b>
								<br><div id ='like'>Like <b>".$like."</b></div>  <div id='dislike'>Dislike <b>".$disLike."</b></div></div>
    								<p>".$body." <br><button onclick =like($storyID)>Like</button> <button onclick =dislike($storyID)>Dislike</button></p>
                                                                <hr/>
							 </section>   		
						   </main>";	
					$i++;
